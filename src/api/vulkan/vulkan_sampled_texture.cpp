...
StatusCode VulkanSampledTexture::create_sampled_texture_api(uint32_t width, uint32_t height, uint32_t depth, uint32_t mip_levels, TextureFormat format)
{
    VkExtent3D vk_extent               = VkExtent3D{ width, height, depth };
    VkImageType vk_type                = vulkan_helpers::find_image_type(vk_extent);
    VkFormat vk_format                 = vulkan_helpers::convert_texture_format(format);
